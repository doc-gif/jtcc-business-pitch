...
タグが指すcommitのサイトをビルドし、`/versions/v1.1.0/` に保存します。スライドの `#slide-1`〜`#slide-7` も各保存版URLで使えます。タグ名の形式が異なる場合、同じ保存パスが異なるcommitですでに使われている場合、ビルドに失敗した場合は既存保存版を変更せずActionsが失敗します。同じタグ・同じcommitの重複実行は保存済みの一致するビルドを再利用します。タグの削除だけでは公開済みページは削除されません。公開を取り消す場合は、管理用 `pages-store` ブランチから該当の `versions/<tag>` ディレクトリと `version-sources/<tag>.sha` を削除し、その後正常にデプロイする必要があります。
